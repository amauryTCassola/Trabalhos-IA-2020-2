...
def compute_cost(theta_0, theta_1, x, y):
    ...
    for i in range(n):
        total_cost += ((theta_0 + (theta_1 * x[i])) - y[i]) ** 2
...
def step_gradient(theta_0_current, theta_1_current, x, y, alpha):
    ...
    Args:
        ...
        x,y (np.array): vetor com dados de treinamento (x,y)
    ...
    for i in range(n-1):
      derivada_0 += theta_0_current + (theta_1_current * x[i]) - y[i]
      derivada_1 += (theta_0_current + (theta_1_current * x[i]) - y[i]) * x[i]
...
def gradient_descent(x, y, starting_theta_0, starting_theta_1, learning_rate, num_iterations):
    ...
    Args:
        x,y (np.array): dados de treinamento, x e y 
    ...
    for i in range(num_iterations):
        ...
        theta_0, theta_1 = step_gradient(theta_0, theta_1, x, y, alpha=0.0001)
...
#Gráfico dos dados
        ...
        theta_0, theta_1, cost_graph, theta_0_progress, theta_1_progress = gradient_descent(x, y, starting_theta_0=0, starting_theta_1=0, learning_rate=0, num_iterations=iter)
        ...
        print ('Erro Quadrático Médio: ', compute_cost(theta_0, theta_1, x, y))
