...
def compute_cost(theta_0, theta_1, theta_2, theta_3, theta_4, theta_5, x, y, z, u, v, w):
    """
    Calcula o erro quadratico medio
    
    Args:
        theta_0 (float): intercepto da reta 
        theta_1 (float): parâmetro multiplica x
        theta_2 (float): parâmetro multiplica z
        theta_3 (float): parâmetro multiplica u
        theta_4 (float): parâmetro multiplica v
        theta_5 (float): parâmetro multiplica w
        
        x,y,z,u,v,w (np.array): colunas     
    ...
    for i in range(n):
      total_cost += ((theta_0 + (theta_1 * x[i]) + (theta_2 * z[i]) + (theta_2 * u[i]) + (theta_2 * v[i]) + (theta_2 * w[i])) - y[i]) ** 2
...
def step_gradient(theta_0_current, theta_1_current, theta_2_current, theta_3_current, theta_4_current, theta_5_current, x, y, z, u, v, w, alpha):
    ...
    Args:
        ...
        theta_3_current (float): valor atual de theta_3
        theta_4_current (float): valor atual de theta_4
        theta_5_current (float): valor atual de theta_5
        data (np.array): vetor com dados de treinamento (x,y,z,u,v,w)
        alpha (float): taxa de aprendizado / tamanho do passo 
    
    Retorna:
        tupla: (theta_0, theta_1) os novos valores de theta_0, theta_1, theta_2, theta_3, theta_4, theta_5
    """
    ...
    for i in range(n-1):
      derivada_0 += (theta_0_current + (theta_1_current * x[i]) + (theta_2_current * z[i]) + (theta_2_current * u[i]) + (theta_2_current * v[i]) + (theta_2_current * w[i])) - y[i]
      derivada_1 += ((theta_0_current + (theta_1_current * x[i]) + (theta_2_current * z[i]) + (theta_2_current * u[i]) + (theta_2_current * v[i]) + (theta_2_current * w[i])) - y[i])*x[i]
      derivada_2 += ((theta_0_current + (theta_1_current * x[i]) + (theta_2_current * z[i]) + (theta_2_current * u[i]) + (theta_2_current * v[i]) + (theta_2_current * w[i])) - y[i])*z[i]
      derivada_3 += ((theta_0_current + (theta_1_current * x[i]) + (theta_2_current * z[i]) + (theta_2_current * u[i]) + (theta_2_current * v[i]) + (theta_2_current * w[i])) - y[i])*u[i]
      derivada_4 += ((theta_0_current + (theta_1_current * x[i]) + (theta_2_current * z[i]) + (theta_2_current * u[i]) + (theta_2_current * v[i]) + (theta_2_current * w[i])) - y[i])*v[i]
      derivada_5 += ((theta_0_current + (theta_1_current * x[i]) + (theta_2_current * z[i]) + (theta_2_current * u[i]) + (theta_2_current * v[i]) + (theta_2_current * w[i])) - y[i])*w[i]
      
    derivada_0=(2/n)*derivada_0
    derivada_1=(2/n)*derivada_1 
    derivada_2=(2/n)*derivada_2 
    derivada_3=(2/n)*derivada_3 
    derivada_4=(2/n)*derivada_4 
    derivada_5=(2/n)*derivada_5 
    ...
    theta_3_updated = theta_3_current - (alpha * derivada_3)
    theta_4_updated = theta_4_current - (alpha * derivada_4)
    theta_5_updated = theta_5_current - (alpha * derivada_5)
...
def gradient_descent(data, starting_theta_0, starting_theta_1, starting_theta_2, starting_theta_3, starting_theta_4, starting_theta_5, learning_rate, num_iterations):
    ...
    Args:
        x,y,z,u,v,w: dados de treinamento x,y,z,u,v,w
        ...
        starting_theta_3 (float): valor inicial de theta3
        starting_theta_4 (float): valor inicial de theta4
        starting_theta_5 (float): valor inicial de theta5
    ...
    for i in range(num_iterations):
        cost_graph.append(compute_cost(theta_0, theta_1, theta_2, theta_3, theta_4, theta_5, x, y, z, u, v, w))
        theta_0, theta_1, theta_2, theta_3, theta_4, theta_5 = step_gradient(theta_0, theta_1, theta_2, theta_3, theta_4, theta_5, x, y, z, u, v, w, alpha=0.0001)
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 3:
        ...
        u = np.array(data[1:,18])
        v = np.array(data[1:,62])
        w = np.array(data[1:,19])
...
#Gráfico dos dados
        ...
        theta_0, theta_1, theta_2, theta_3, theta_4, theta_5, cost_graph, theta_0_progress, theta_1_progress, theta_2_progress, theta_3_progress, theta_4_progress, theta_5_progress = gradient_descent(data, starting_theta_0=0, starting_theta_1=0, starting_theta_2=0, starting_theta_3=0, starting_theta_4=0, starting_theta_5=0, learning_rate=0, num_iterations=iter)
        ...
        print ('Erro Quadrático Médio: ', compute_cost(theta_0, theta_1, theta_2, theta_3, theta_4, theta_5, x, y, z, u, v, w))
